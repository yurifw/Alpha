...
def key_expand(key, size):
    ...
    def rijndael_sbox(byte):
        ...
        row = byte & 15
        return s_box[col][row]
    def rijndael_expansion(key,size):
        rcon = bytearray()
        ...
        return key
    pre_key = list(rijndael_expansion(key, size))
    master_key = []

    for i in range(0,size,16):
        h = hashlib.new('md5')
        h.update(bytearray(pre_key[i: i+16]))
        master_key.extend(array.array('B', h.digest())) #o array.array transforma a string retornada por h.digest em um array de int
    return master_key
...
def encipher_block(text, key, rounds, block_size, encipher, current_block):
    """encipher or decipher the given text (True to encipher and False to decipher), using the given key(not expanded)
    the algorithm will do nr rounds. The rounds should also be 16
    """
